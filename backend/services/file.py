...
from exceptions.exceptions import NotFoundError, InvalidFileUploadConfirm
...
class FileService:
    ...
    async def confirm_point_cloud_upload(
        ...
        try:
            ...
            # check file status
            if file_db.status != FileStatus.PENDING:
                raise InvalidFileUploadConfirm(
                    "file status not pending, no confirm awaited")
