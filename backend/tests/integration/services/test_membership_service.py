...
from core.exceptions import NotMemberError, ProhibitedWorkspaceActionError, MembershipViolationError
from core.roles import Role
from schemas.membership import MembershipModel
from services.membership import get_workspace_members, get_membership, create_membership, delete_membership, change_user_role
...
async def test_get_membership(db_session):
    ...
    workspace = await create_test_workspace(db_session)
    user = await create_test_user(db_session)
    created_membership = await create_test_membership(db_session, workspace_id=workspace.id, user_id=user.id, role=Role.MEMBER)
...
async def test_get_workspace_members(db_session):
    ...
    membership1 = await create_test_membership(db_session, workspace_id=workspace.id, user_id=user1.id, role=Role.MEMBER)
    membership2 = await create_test_membership(db_session, workspace_id=workspace.id, user_id=user2.id, role=Role.MEMBER)
...
async def test_delete_membership(db_session):
    ...
    membership = await create_test_membership(db_session, workspace_id=workspace.id, user_id=user.id, role=Role.MEMBER)

    deleted_membership = await delete_membership(user.id, workspace.id, session=db_session)
...
async def test_delete_owner_membership(db_session):
    ...
    _ = await create_test_membership(db_session, workspace_id=workspace.id, user_id=user.id, role=Role.OWNER)
...
async def test_change_user_role(db_session):
    ...
    _ = await create_test_membership(db_session, workspace_id=workspace.id, user_id=editor.id, role=Role.OWNER)
    _ = await create_test_membership(db_session, workspace_id=workspace.id, user_id=user.id, role=Role.MEMBER)
...
async def test_change_owner_role(db_session):
    ...
    editor = await create_test_user(db_session, email="test@test.com")
    user = await create_test_user(db_session, email="test2@test.com")
    _ = await create_test_membership(db_session, workspace_id=workspace.id, user_id=editor.id, role=Role.ADMIN)
    _ = await create_test_membership(db_session, workspace_id=workspace.id, user_id=user.id, role=Role.OWNER)
