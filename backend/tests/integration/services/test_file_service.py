...
from schemas.file import BIMModel, FileDataRequest, FileModel, FileStatus, PointCloudType
from services.file import FileService

from tests.helpers import create_test_workspace, create_test_project, create_test_stage, create_test_file, create_test_point_cloud, create_test_bim
...
@pytest.mark.asyncio
async def test_clean_up_files(db_session, storage, test_building_ifc_path, test_building_laz_path):
    ...
    uploaded_file = await create_test_file(db_session, workspace.id, file_path=test_building_ifc_path)
    ...
    assert await FileRepository.get_by_id(uploaded_file.id, session=db_session) is not None
    ...
    old_pending_file = await create_test_file(db_session, workspace.id, file_path=test_building_laz_path)
    old_pending_file.created_at = old_pending_file.created_at - \
        timedelta(days=2)
    assert old_pending_file.status == FileStatus.PENDING
    assert await FileRepository.get_by_id(old_pending_file.id, session=db_session) is not None
    ...
    recent_pending_file = await create_test_file(db_session, workspace.id, file_path=test_building_laz_path)
    assert recent_pending_file.created_at > old_pending_file.created_at
    assert recent_pending_file.status == FileStatus.PENDING
    assert await FileRepository.get_by_id(recent_pending_file.id, session=db_session) is not None
    ...
    assert await FileRepository.get_by_id(old_pending_file.id, session=db_session) is None
...
async def test_check_file_meta(db_session, test_building_ifc_path):
    ...
    workspace = await create_test_workspace(db_session)
    file = await create_test_file(db_session, workspace_id=workspace.id, file_path=test_building_ifc_path)
...
async def test_get_point_cloud_raises_for_nonexistent_id(db_session):
    ...
    assert await FileService.get_point_cloud_by_file_id(uuid.uuid4(), session=db_session) is None


@pytest.mark.asyncio
async def test_get_bim_by_id_and_file_id_and_project_id(db_session, test_building_ifc_path):
...
async def test_get_bim_raises_for_nonexistent_id(db_session):
    ...
    assert await FileService.get_bim_by_file_id(uuid.uuid4(), session=db_session) is None

    assert await FileService.get_bim_by_project_id(uuid.uuid4(), session=db_session) is None
...
async def test_generate_bim_upload_link(db_session, storage, test_building_ifc_path):
    ...
    assert link.startswith(
        f"http://localhost:{storage_port}") or link.startswith(f"https://localhost:{storage_port}")
    assert file.filename == test_building_ifc_path.name
    assert file.size == test_building_ifc_path.stat().st_size
    assert file.content_type == "application/x-ifc"
...
@pytest.mark.asyncio
async def test_generate_point_cloud_upload_link(db_session, storage, test_building_laz_path):
...
@pytest.mark.asyncio
async def test_save_and_get_converted_point_cloud_files(db_session, test_building_laz_path, test_report_xlsx_path, test_report_pdf_path):
    ...
    assert len(
        converted_files) == 2, "Should return two converted files for the point cloud"
