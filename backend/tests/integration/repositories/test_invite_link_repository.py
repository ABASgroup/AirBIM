...
async def test_invite_link_repository_create_and_get_by_token(db_session):
    ...
    created_invite_link = await InviteLinkRepository.create(invite_link_data, db_session)
...
async def test_invite_link_repository_delete_by_workspace_id(db_session):
    ...
    await InviteLinkRepository.delete_by_workspace_id(workspace_id=workspace.id, session=db_session)
