...
from repositories.files import FileRepository, BIMRepository, PointCloudRepository, PointCloudConvertedRepository
...
async def test_file_repository_update_status(db_session, test_building_shifted_laz_path):
    ...
    workspace = await create_test_workspace(db_session)
    file_payload = FileService.collect_file_data(
...
@pytest.mark.asyncio
async def test_file_repository_get_all_keys_returns_all_file_keys(db_session, test_building_ifc_path, test_building_laz_path):
...
@pytest.mark.asyncio
async def test_file_repository_create_persists_pending_file(db_session, test_building_laz_path):
...
@pytest.mark.asyncio
async def test_bim_repository_create_and_get_by_project_and_file_id(db_session, test_building_ifc_path):
    """Repository should return all file keys."""
    workspace = await create_test_workspace(db_session)
    ...
    assert created_bim.project_id == project.id, "BIM should be created with correct project_id"
    assert created_bim.file_id == bim_file.id, "BIM should be created with correct project_id and file_id"
    ...
    assert fetched_bim.id == created_bim.id, "Fetched BIM should have the same id as created"
...
@pytest.mark.asyncio
async def test_bim_repository_set_point_cloud(db_session, test_building_ifc_path, test_building_laz_path):
    ...
    created_point_cloud = await PointCloudRepository.create(point_cloud, session=db_session)
    created_bim = await BIMRepository.create(bim, session=db_session)

    updated_bim = await BIMRepository.set_point_cloud(created_bim, created_point_cloud.id, session=db_session)
    assert updated_bim.point_cloud_id == created_point_cloud.id, "BIM should be updated with correct point cloud id"

    fetched_bim = await BIMRepository.get_by_project_id(project.id, session=db_session)
    assert fetched_bim is not None, "BIM should be fetched by project_id"
    assert fetched_bim.point_cloud_id == created_point_cloud.id, "Fetched BIM should have the same point cloud id as updated"


@pytest.mark.asyncio
async def test_point_cloud_repository_create_get_by_file_id(db_session, test_building_laz_path):
    ...
    assert created_point_cloud.file_id == point_cloud_file.id, "Point cloud should be created with correct file_id"

    fetched_point_cloud = await PointCloudRepository.get_by_file_id(point_cloud_file.id, session=db_session)
    assert fetched_point_cloud is not None, "Point cloud should be fetched by file_id"
    assert fetched_point_cloud.id == created_point_cloud.id, "Fetched point cloud should have the same id as created"


@pytest.mark.asyncio
async def test_point_cloud_converted_repository_create_and_get_by_point_cloud_id(db_session, test_building_laz_path, test_building_shifted_laz_path):
    ...
    for converted_file in [converted_file_1, converted_file_2]:
        ...
        created_converted = await PointCloudConvertedRepository.create(point_cloud_converted, session=db_session)
        assert created_converted.id is not None, "Created point cloud converted should have an id"
        assert created_converted.point_cloud_id == created_point_cloud.id, "Point cloud converted should be created with correct point cloud id"
        assert created_converted.file_id == converted_file.id, "Point cloud converted should be created with correct file id"

    fetched_converted = await PointCloudConvertedRepository.get_by_point_cloud_id(created_point_cloud.id, session=db_session)
    assert len(fetched_converted) == 2, "There should be 2 converted point clouds fetched by point cloud id as 2 were created"
    for converted in fetched_converted:
        assert converted is not None, "Point cloud converted should be fetched by point cloud id"
        assert converted.point_cloud_id == created_point_cloud.id, "Fetched point cloud converted should have the same point cloud id as created"
